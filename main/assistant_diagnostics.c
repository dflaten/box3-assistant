...
esp_err_t assistant_diag_init(void) {
    ...
    if (err == ESP_OK) {
        ...
        if (err == ESP_OK && blob_size == sizeof(blob) && blob.magic == ASSISTANT_DIAG_MAGIC &&
            blob.version == ASSISTANT_DIAG_VERSION && blob.size == sizeof(blob)) {
...
void assistant_diag_update_detail(uint8_t assistant_stage,
    ...
    assistant_diag_write_locked(
        assistant_stage, detail_stage, weather_day, attempt, last_err, s_record.active, s_record.timed_out);
    xSemaphoreGive(s_diag_mutex);
}
...
void assistant_diag_finish_command(esp_err_t last_err) {
    ...
    assistant_diag_write_locked(
        0, ASSISTANT_DIAG_DETAIL_FINISH, s_record.weather_day, s_record.attempt, last_err, false, s_record.timed_out);
    xSemaphoreGive(s_diag_mutex);
}
...
bool assistant_diag_format_previous_issue(char *buffer, size_t buffer_size) {
    ...
    bool suspicious_reset = s_previous_reset_reason == ESP_RST_PANIC || s_previous_reset_reason == ESP_RST_INT_WDT ||
                            s_previous_reset_reason == ESP_RST_TASK_WDT || s_previous_reset_reason == ESP_RST_WDT;
...
void assistant_diag_log_previous_issue(void) {
    ...
    ESP_LOGI(TAG,
             "Previous assistant state: reset=%s active=%s timeout=%s command_id=%ld detail=%s err=%s attempt=%lu "
             "uptime_ms=%lu free_heap=%lu largest_block=%lu",
