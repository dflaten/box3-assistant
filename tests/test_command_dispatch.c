...
static bool test_command_dispatch_resolves_builtin_and_hue_actions(void) {
    ...
    assistant_command_resolve(ASSISTANT_CMD_VOLUME_UP_BASE, 3, &dispatch);
    ASSERT_EQ_INT(ASSISTANT_COMMAND_ACTION_VOLUME_UP, dispatch.type);
    ASSERT_EQ_INT(1, dispatch.volume_steps);

    assistant_command_resolve(ASSISTANT_CMD_VOLUME_UP_BASE + 9, 3, &dispatch);
    ASSERT_EQ_INT(ASSISTANT_COMMAND_ACTION_VOLUME_UP, dispatch.type);
    ASSERT_EQ_INT(10, dispatch.volume_steps);

    assistant_command_resolve(ASSISTANT_CMD_VOLUME_DOWN_BASE + 4, 3, &dispatch);
...
static bool test_command_dispatch_unknown_for_out_of_range_ids(void) {
    ...
    ASSERT_EQ_INT(0, dispatch.volume_steps);
    ASSERT_TRUE(!dispatch.on);
